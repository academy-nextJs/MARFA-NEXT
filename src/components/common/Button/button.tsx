...
interface ButtonEmptyProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  className?: string;
}

export const ButtonEmpty: FC<ButtonEmptyProps> = ({ children, className, ...props }) => {
  return (
    <button
      className={`p-1 rounded-full transition-all duration-200 hover:bg-gray-200 ${className}`}
      {...props}
    >
      {children}
    </button>
