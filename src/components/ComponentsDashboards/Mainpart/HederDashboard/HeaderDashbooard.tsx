...
const HeaderDashboard: React.FC = () => {
  ...
  return (
    <Fragment>
      <div className="bg-[#ffffff] dark:bg-gray-800 w-full rounded-[12px] px-8 py-3 flex items-center justify-between">
        {routeSelect.map(({ label, href }) => {
          return (
            pathname === href && (
              <div key={label} className="font-extrabold text-xl">
                {label}
              </div>
            )
          );
